...
final class SupabaseMappingTests: XCTestCase {
    ...
    func testCardDesignsHaveStableDatabaseValues() throws {
        XCTAssertEqual(
            ProfileCardDesign.allCases.map(\.rawValue),
            ["classic", "cartridge", "console", "arcade", "neon", "cosmos"]
        )

        for design in ProfileCardDesign.allCases {
            let data = try JSONEncoder().encode(design)
            XCTAssertEqual(try JSONDecoder().decode(ProfileCardDesign.self, from: data), design)
            XCTAssertNotNil(UIImage(named: design.assetName), "Missing generated card asset: \(design.assetName)")
        }
    }

    func testProfileCardDefaultsToClassicWithoutMatches() {
        let card = ProfileCard(username: "Player", userId: UUID().uuidString)

        XCTAssertEqual(card.cardDesign, .classic)
        XCTAssertTrue(card.matchingOwnedGames.isEmpty)
    }
    ...
    func testTextMessageRPCEncodesNullableImageParameter() throws {
        let params = SendMessageParams(
            messageId: UUID().uuidString,
            targetChatId: UUID().uuidString,
            messageText: "Hello",
            messageImageUrl: nil
        )
        ...
        XCTAssertEqual(object["message_text"] as? String, "Hello")
        XCTAssertTrue(object.keys.contains("message_image_url"))
        XCTAssertTrue(object["message_image_url"] is NSNull)
    }
}
