...
2. В DataGrip откройте готовое подключение `UniShare — удалённая база`.
...
Готовые русские запросы находятся в
`/Users/egorchulanov/Desktop/UniShare/datagrip`. Начинайте с `00_health.sql`.

## Истории

### Загрузка фотографии

SQL хранит только ссылку, поэтому сам файл сначала загружается в Storage:

1. Откройте Supabase Dashboard -> `Storage` -> `story-media`.
2. Нажмите `Upload file` и выберите квадратный JPG, PNG или WebP до 15 МБ.
3. Используйте уникальное имя файла, чтобы старая CDN-копия не подменила новую.
4. Откройте файл и нажмите `Copy URL` / `Get URL`.
5. В DataGrip откройте `21_stories_admin.sql` и вставьте ссылку в `IMAGE_URL`.

Рекомендуемый размер изображения: 1200 x 1200. Ссылка должна начинаться с
...
insert into public.stories (
    ...
    'https://kwonpzkzthprilrhncik.supabase.co/storage/v1/object/public/story-media/stories/2026-08/letniy-turnir.webp',
    'gamecontroller.fill',
    '0057FF',
    'Открыть главное',
    'unishare://feed',
...
RAWG API key хранится зашифрованным в Supabase Vault. Его можно создать или
заменить одним запуском `datagrip/51_установить_ключ_rawg.sql`. Проверочный запрос
показывает только факт настройки ключа и никогда не выводит его значение.
