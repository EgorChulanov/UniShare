...
struct UserProfile: Codable, Identifiable {
    ...
    var firestoreData: [String: Any] {
        ...
        if let lastSeen { data["lastSeen"] = lastSeen }
        return data
    }

    static func from(_ data: [String: Any], uid: String) -> UserProfile? {
        guard let username = data["username"] as? String else { return nil }
        var profile = UserProfile(uid: uid, username: username)
        profile.avatarUrl = data["avatarUrl"] as? String
        profile.status = data["status"] as? String
        ...
        if let subs = data["subscriptions"] as? [[String: String]] {
            profile.subscriptions = subs.compactMap { dict in
                ...
                return LocalUserSubscription(name: name, url: dict["url"], iconName: iconName)
            }
        }
        return profile
    }
