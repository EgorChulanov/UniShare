...
final class AuthViewModel: ObservableObject {
    ...
    init(auth: SupabaseAuthService) {
        ...
        if environment["UNISHARE_FORCE_REGISTRATION"] == "1" {
            isLoginMode = false
        }
        if environment["UNISHARE_UI_TESTING"] == "1",
           environment["UNISHARE_E2E_REGISTRATION"] == "1",
           let testEmail = environment["UNISHARE_E2E_EMAIL"],
           let testPassword = environment["UNISHARE_E2E_PASSWORD"] {
            email = testEmail
