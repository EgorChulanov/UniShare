...
struct ChatsView: View {
    ...
    init() {
        ...
        _vm = StateObject(wrappedValue: ChatsViewModel(auth: env.auth, db: env.db, storage: env.storage))
    }
    ...
    var body: some View {
        NavigationView {
            ...
            .fullScreenCover(item: $selectedStory) { story in
                CommunityStoryViewer(stories: vm.stories, initialStoryID: story.id)
                    .environmentObject(theme)
            }
    ...
    private func requestRow(_ request: LikeRequest) -> some View {
        ...
        return HStack(spacing: 13) {
            ...
            .disabled(vm.processingRequestIds.contains(request.id))
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 20) {
                        ...
                        if !profile.skillsPortfolioUrls.isEmpty {
                            SkillsPortfolioGallery(urls: profile.skillsPortfolioUrls)
                        }
                    }
                    .padding(.bottom, 40)
                }
