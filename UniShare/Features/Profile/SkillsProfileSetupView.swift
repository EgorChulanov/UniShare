...
struct SkillsProfileSetupView: View {
    ...
    private func saveAndFinish() async {
        ...
        do {
            ...
            for image in portfolioPhotos {
                let url = try await AppEnvironment.shared.storage.uploadSkillsPortfolioImage(
                    image,
                    uid: existingProfile.uid
                )
                uploadedURLs.append(url)
            }
            updated.skillsPortfolioUrls = uploadedURLs

            let data: [String: AnyEncodable] = [
                "skills": AnyEncodable(updated.skills),
                "skills_description": AnyEncodable(updated.skillsDescription ?? ""),
                "skills_portfolio_urls": AnyEncodable(updated.skillsPortfolioUrls),
