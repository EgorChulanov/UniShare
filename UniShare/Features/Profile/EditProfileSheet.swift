...
struct EditProfileSheet: View {
    ...
    @State private var showAvatarCatalog = false
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 24) {
                        ...
                        VStack(spacing: 10) {
                            ZStack(alignment: .bottomTrailing) {
                                Group {
                                    ...
                                    } else if let selectedURL = vm.editAvatarPresetURL {
                                        AvatarView(url: selectedURL, size: 90)
                                    } else {
                                ...
                                Task {
                                    if let data = try? await item?.loadTransferable(type: Data.self),
                                       let img = UIImage(data: data) {
                                        vm.editAvatar = img
                                        vm.editAvatarPresetURL = nil
                                    }
                                }
                            }

                            Button("profile.avatar.catalog.choose".localized) {
                                showAvatarCatalog = true
                            }
                        ...
                        editField(title: "profile.card.design".localized) {
                            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                                ForEach(ProfileCardDesign.allCases) { design in
                                    Button {
                                        vm.editCardDesign = design
                                        HapticsManager.shared.impact(.light)
                                    } label: {
                                        CardDesignOption(design: design, selected: vm.editCardDesign == design)
                                    }
                                    .buttonStyle(.plain)
                                }
                            }
            ...
            .sheet(isPresented: $showAvatarCatalog) {
                NavigationView {
                    ScrollView {
                        AvatarCatalogGrid(
                            presets: vm.avatarPresets,
                            selectedURL: vm.editAvatarPresetURL
                        ) { preset in
                            vm.editAvatar = nil
                            vm.editAvatarPresetURL = preset.imageUrl
                            showAvatarCatalog = false
                        }
                        .padding(16)
                    }
                    .background(theme.effectiveBackground.ignoresSafeArea())
                    .navigationTitle("profile.avatar.catalog".localized)
                    .navigationBarTitleDisplayMode(.inline)
                }
                .environmentObject(theme)
            }
                ...
                if let activePlatform = vm.editActiveGamePlatform {
                    ...
                    HStack {
                        ...
                        TextField("onboarding.games.search".localized, text: $vm.gameSearchQuery)
                            .foregroundColor(theme.effectiveTextColor)
                            .autocapitalization(.none)
                            .onChange(of: vm.gameSearchQuery) { query in vm.searchGames(query) }
...
private struct CardDesignOption: View {
    ...
    var body: some View {
        VStack(spacing: 9) {
            ZStack {
                CardDesignBackground(design: design)
                Image(systemName: design.icon)
                    .font(.system(size: 22, weight: .bold))
                    .foregroundStyle(design.foregroundColor(for: colorScheme))
            }
            .frame(height: 74)
        ...
        .padding(8)
        .background(theme.effectiveCardColor, in: RoundedRectangle(cornerRadius: 20, style: .continuous))
        .overlay {
