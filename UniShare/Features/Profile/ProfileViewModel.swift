...
final class ProfileViewModel: ObservableObject {
    ...
    @Published var editAvatarPresetURL: String?
    @Published var avatarPresets: [AvatarPreset] = []
    @Published var editCardDesign: ProfileCardDesign = .classic
    ...
    func startEditing() {
        ...
        editAvatar = nil
        editAvatarPresetURL = nil
        editCardDesign = p.cardDesign
        isEditing = true
        Task { await loadAvatarPresets() }
    }
    ...
    func saveChanges() async -> Bool {
        ...
        do {
            if let selectedURL = editAvatarPresetURL {
                p.avatarUrl = selectedURL
                await AvatarCacheService.shared.loadUserAvatar(from: selectedURL)
            } else if let image = editAvatar {
                let url = try await storage.uploadAvatar(image, uid: uid)
                p.avatarUrl = url
                AvatarCacheService.shared.store(image, for: url)
            ...
            let data: [String: AnyEncodable] = [
                ...
                "card_design": AnyEncodable(p.cardDesign.rawValue)
            ]
    ...
    func loadAvatarPresets() async {
        guard avatarPresets.isEmpty else { return }
        do {
            avatarPresets = try await db.getAvatarPresets()
        } catch {
            errorMessage = error.localizedDescription
        }
    }
