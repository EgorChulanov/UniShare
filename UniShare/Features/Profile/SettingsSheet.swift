...
struct SettingsSheet: View {
    ...
    var body: some View {
        NavigationView {
            ZStack {
                ...
                List {
                    ...
                    Section("settings.about".localized) {
                        Link(destination: AppConstants.Legal.privacyPolicy(language: localization.currentLanguage)) {
                            Label("settings.privacy".localized, systemImage: "hand.raised.fill")
                        }
                        .listRowBackground(theme.effectiveCardColor)
                        Link(destination: AppConstants.Legal.terms(language: localization.currentLanguage)) {
                    ...
                    Section("settings.storage".localized) {
                        Button {
                            ...
                            HStack {
                                Label("settings.cache.clear".localized, systemImage: "externaldrive.badge.xmark")
                                Spacer()
                                if isClearingCache {
            ...
            .task { cacheSize = await UniShareCacheController.size() }
    ...
    private func clearCache() {
        ...
        Task {
            do {
                try await UniShareCacheController.clear()
                cacheSize = 0
            } catch {
                deletionError = error.localizedDescription
            }
