...
final class SupabaseAuthService: ObservableObject {
    ...
    init() {
        let shouldResetSession = AppConstants.isUITesting
            && ProcessInfo.processInfo.environment["UNISHARE_RESET_SESSION"] == "1"

        // Seed initial state from the cached session synchronously
        let cachedUserID = client.auth.currentUser?.id.uuidString
        self.currentUserID = shouldResetSession ? nil : cachedUserID
        self.isAuthenticated = shouldResetSession ? false : cachedUserID != nil
        ...
        authListenerTask = Task { [weak self] in
            ...
            for await (event, session) in self.client.auth.authStateChanges {
                ...
                await MainActor.run { [weak self] in
                    self?.currentUserID = authenticated ? session?.user.id.uuidString : nil
