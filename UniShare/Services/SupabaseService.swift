...
private struct UserRow: Codable {
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case skillsPortfolioUrls = "skills_portfolio_urls"
    ...
    func toUserProfile() -> UserProfile {
        ...
        profile.skillsPortfolioUrls = skillsPortfolioUrls ?? []
        ...
        profile.cardDesign = cardDesign ?? .classic
        return profile
    }
...
private struct ProfileSearchParams: Encodable {
...
private struct DeleteChatParams: Encodable {
    let targetChatId: String
...
private struct StoryRow: Decodable {
    ...
    func toStory(isSeen: Bool) -> CommunityStory {
        let fallbackSlide = CommunityStorySlide(
            title: title,
            subtitle: subtitle,
            body: body,
            imageUrl: imageUrl,
        ...
        return CommunityStory(
            ...
            slides: (slides?.isEmpty == false) ? (slides ?? []) : [fallbackSlide],
            isSeen: isSeen
        )
    }
}
...
private final class CachedProfileBox {
    let profile: UserProfile
    let loadedAt: Date

    init(profile: UserProfile, loadedAt: Date = Date()) {
...
final class SupabaseService {
    ...
    private let profileCache = NSCache<NSString, CachedProfileBox>()
    ...
    func createUser(_ profile: UserProfile) async throws {
        ...
        try await client.from("users")
            ...
            .execute()
        profileCache.setObject(CachedProfileBox(profile: profile), forKey: profile.uid as NSString)
    }

    func getUser(uid: String, forceRefresh: Bool = false) async throws -> UserProfile? {
        if !forceRefresh,
           let cached = profileCache.object(forKey: uid as NSString),
           Date().timeIntervalSince(cached.loadedAt) < profileCacheTTL {
        ...
        let rows: [UserRow] = try await client.from("users")
            ...
            .execute()
            .value
        let profile = rows.first?.toUserProfile()
        if let profile {
            profileCache.setObject(CachedProfileBox(profile: profile), forKey: uid as NSString)
        } else {
            profileCache.removeObject(forKey: uid as NSString)
        }
        return profile
    }
    ...
    func getAvatarPresets() async throws -> [AvatarPreset] {
        try await client.from("avatar_presets")
            .select()
    ...
    func searchProfiles(query: String, limit: Int = 30) async throws -> [UserProfile] {
        let normalized = query.trimmingCharacters(in: .whitespacesAndNewlines)
        guard normalized.count >= 2 else { return [] }

        let rows: [UserRow] = try await client.rpc(
            "search_profiles",
            params: ProfileSearchParams(searchText: normalized, resultLimit: limit)
        )
        .execute()
        .value
        return rows.map { $0.toUserProfile() }
    ...
    func listenToUserStatus(uid: String, completion: @escaping (Bool) -> Void) -> () -> Void {
        let task = Task {
            while !Task.isCancelled {
                if let profile = try? await getUser(uid: uid, forceRefresh: true) {
    ...
    func deleteChat(id: String) async throws {
        try await client.rpc(
            "delete_chat",
            params: DeleteChatParams(targetChatId: id)
        )
        .execute()
    }

    func chatImagePaths(chatId: String) async throws -> [String] {
        let rows: [MessageRow] = try await client.from("messages")
            .select()
            .eq("chat_id", value: chatId)
            .execute()
            .value
        return rows.compactMap(\.imageUrl).filter { !$0.isEmpty && !$0.hasPrefix("http") }
    }
