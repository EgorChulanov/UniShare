...
final class SupabaseStorageService {
    ...
    func uploadAvatar(_ image: UIImage, uid: String) async throws -> String {
        ...
        // UUID strings coming from Foundation are uppercase while auth.uid() is
        // rendered lowercase by Postgres. Keep object ownership paths canonical.
        let path = "\(uid.lowercased())/avatar.jpg"
        do {
            ...
            if message.contains("row-level security") || message.contains("policy") {
                throw StorageError.avatarPermissionDenied
            }
            throw error
        }
    }
    ...
    func uploadSkillsPortfolioImage(_ image: UIImage, uid: String) async throws -> String {
        guard let data = image.preparingForUpload(maxDimension: 1_600, quality: 0.76) else {
            throw StorageError.compressionFailed
        }
        let path = "\(uid.lowercased())/\(UUID().uuidString.lowercased()).jpg"
        _ = try await client.storage
            .from("skills-portfolio")
            .upload(path, data: data, options: FileOptions(contentType: "image/jpeg", upsert: false))
        return try client.storage.from("skills-portfolio").getPublicURL(path: path).absoluteString
    }
    ...
    func removeChatImages(paths: [String]) async throws {
        let uniquePaths = Array(Set(paths.filter { !$0.isEmpty }))
        guard !uniquePaths.isEmpty else { return }
        _ = try await client.storage.from("chats").remove(paths: uniquePaths)
    }
    ...
    enum StorageError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .avatarPermissionDenied: return "profile.error.avatar.permission".localized
            }
        }
    }
}
