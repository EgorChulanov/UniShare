...
add column if not exists card_design text not null default 'classic';

alter table public.users
drop constraint if exists users_card_design_check;

alter table public.users
add constraint users_card_design_check
check (card_design in ('classic', 'cartridge', 'console', 'arcade', 'neon', 'cosmos'));
...
begin
    ...
    order by (
        case when candidate.games && viewer.wanted_games then 10 else 0 end
        + case when candidate.wanted_games && viewer.games then 8 else 0 end
