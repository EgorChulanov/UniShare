...
# db reset recreates upstream containers. Restart Kong so it does not retain a
# stale Auth container address from the previous local stack.
...
run_bounded 30 sh -c '
    until curl -fsS "$1/auth/v1/settings" -H "apikey: $2" >/dev/null; do sleep 1; done
' sh "$UNISHARE_E2E_URL" "$UNISHARE_E2E_KEY" || {
    echo "Local Supabase Auth did not become ready after reset" >&2
    exit 1
}
export DEVELOPER_DIR=${DEVELOPER_DIR:-/Applications/Xcode-beta.app/Contents/Developer}
DERIVED_DATA=${UNISHARE_DERIVED_DATA:-"${TMPDIR:-/tmp}/UniShareDerivedData/LocalE2E"}

DEVICE_ID=$(xcrun simctl list devices available -j | jq -r '
    [.devices[][] | select(.name == "UniShare QA Fresh")][0].udid //
...
run_bounded 900 xcodebuild build-for-testing \
...
for TEST_NAME in \
    testFullRegistrationProfileAndDeletionAgainstLocalSupabase \
    testMutualMatchAndChatAgainstLocalSupabase
do
    run_bounded 20 xcrun simctl shutdown "$DEVICE_ID" 2>/dev/null || true
    run_bounded 20 xcrun simctl boot "$DEVICE_ID" 2>/dev/null || true
    run_bounded 120 xcrun simctl bootstatus "$DEVICE_ID" -b || {
        echo "CoreSimulator did not boot within 120 seconds. Restart macOS before running UI E2E." >&2
        exit 1
    }
    ...
    run_bounded 600 xcodebuild test-without-building \
        ...
        -only-testing:"UniShareUITests/LaunchUITests/$TEST_NAME" \
        "UNISHARE_E2E_URL=$UNISHARE_E2E_URL" \
        "UNISHARE_E2E_KEY=$UNISHARE_E2E_KEY"
done
